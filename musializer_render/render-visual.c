...
{
    memset(r->in_raw, 0, sizeof(r->in_raw));
    memset(r->in_win, 0, sizeof(r->in_win));
    memset(r->out_raw, 0, sizeof(r->out_raw));
    memset(r->out_log, 0, sizeof(r->out_log));
    memset(r->out_smooth, 0, sizeof(r->out_smooth));
    memset(r->out_smear, 0, sizeof(r->out_smear));
}
...
int main(int argc, char *argv[])
{
    ...
    r->font = LoadFontEx("./resources/fonts/Alegreya-Regular.ttf", 64, NULL, 0);
    GenTextureMipmaps(&r->font.texture);
    SetTextureFilter(r->font.texture, TEXTURE_FILTER_BILINEAR);

    r->circle = LoadShader(NULL, TextFormat("./resources/shaders/glsl%d/circle.fs", GLSL_VERSION));
